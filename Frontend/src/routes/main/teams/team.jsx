...
export default function Team() {
  ...
  const unsendRequest=async ()=>{
    try {
      const res=await axios.put(`http://localhost:3000/teams/${params.teamId}/unsend/${sessionStorage.getItem("userId")}`, {headers: {"Authorization": `Bearer ${sessionStorage.getItem("accessToken")}`}});
  ...
  const getTeam=async () =>{
    try {
      const res=await axios.get(`http://localhost:3000/teams/${params.teamId}`, {headers: {"Authorization": `Bearer ${sessionStorage.getItem("accessToken")}`}});
