...
const addTeam = async (req, res) => {
    try {
        ...
        const result=await teamAuthSchema.validateAsync(req.body);

        //checking if email or phone already registered
        const emailExists=await Team.findOne({email: result.email});
        if(emailExists){
            return res.status(409).json({message: "Email already exists."});//Conflict
        }
        const phoneExists=await Team.findOne({phone: result.phone});
        if(phoneExists){
        ...
        const id=result.teamName[0].toLowerCase()+result.teamName[1].toLowerCase()+"-"+(teams+1);

        //creating new team
        const team=await Team.create({
            teamId: id,
            teamName: result.teamName,
            establishedInYear: result.establishedInYear,
            phone: result.phone,
            email: result.email,
            profileImage: result.profileImage,
            coverImage: result.coverImage,
            facebookHandle: result.facebookHandle,
            instaHandle: result.instaHandle,
            captain: user._id,
...
const updateTeam = async (req, res) => {
    try {
        ...
        //checking if user is captain of team
        if(user.role=="Captain" && !user._id.equals(team.captain)){
            return res.status(403).json({message: "Only admin and captain of team allowed to make changes"});//Forbidden
        }
        ...
        if(req.body.hasOwnProperty('email') && await Team.findOne({email: req.body.email})){
            return res.status(400).json({message: "This email already exists."});
        }

        //checking if phone number to be updated already exists
        if(req.body.hasOwnProperty('phone') && await Team.findOne({phone: req.body.phone})){
