...
export default function Team() {
  ...
  const sendRequest=async ()=>{
    try {
      const res=await axios.put(`http://localhost:3001/teams/${params.id}/send/${sessionStorage.getItem("userId")}`, {headers: {"Authorization": `Bearer ${sessionStorage.getItem("accessToken")}`}});
  ...
  const unsendRequest=async ()=>{
    try {
      const res=await axios.put(`http://localhost:3001/teams/${params.id}/unsend/${sessionStorage.getItem("userId")}`, {headers: {"Authorization": `Bearer ${sessionStorage.getItem("accessToken")}`}});
  ...
  const leaveTeam=async ()=>{
    try {
      const res=await axios.put(`http://localhost:3001/teams/${params.id}/leaveTeam/${sessionStorage.getItem("userId")}`, {headers: {"Authorization": `Bearer ${sessionStorage.getItem("accessToken")}`}});
  ...
  const getTeam=async () =>{
    try {
      const res=await axios.get(`http://localhost:3001/teams/${params.id}`, {headers: {"Authorization": `Bearer ${sessionStorage.getItem("accessToken")}`}});
      console.log(res.data);
      setTeam(res.data);

      // check if logged in user is part of players
      if(res.data.players && res.data.players.some((player)=>player._id===sessionStorage.getItem("_id"))){
        setButton("Leave Team")
      }

      // check if logged in user has already sent request
      if(res.data.requests && res.data.requests.some((request)=>request._id===sessionStorage.getItem("_id"))){
        setButton("Request Sent")
      }
  ...
  return (
    <Page>
        ...
        <Content>
          ...
          {sessionStorage.getItem("userId") && request==false ? 

          (team.captain && sessionStorage.getItem("userId")==team.captain.userId ? 
          ...
          (team.players && team.players.some((player)=>player._id===sessionStorage.getItem("_id")) ? 

          <MUIButton onClick={()=>leaveTeam()} disabled={false} title="Leave Team" color="success" style={{width: "170px", margin: "0 0 0 auto", position: "relative", bottom: "52px"}}/> :
